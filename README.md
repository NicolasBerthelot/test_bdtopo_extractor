...
- **`fields.py`** : métadonnées des attributs filtrables par couche (nom, type, valeurs possibles), scrapées sur bdtopoexplorer.ign.fr et croisées avec le schéma réel (`pyogrio.read_info`). Même principe d'instantané embarqué que `docs.py`. Dans l'UI, chaque couche cochée avec des champs filtrables propose un panneau "⚙ Filtres" (seuil min/max sur les champs quantitatifs, cases à cocher sur les champs à modalités), appliqué après extraction via `extract.apply_filters()`. **Le filtre ne réduit que le résultat, pas la lecture réseau** : FlatGeobuf ne permet de filtrer qu'à la lecture spatialement, pas par attribut — filtrer un champ rare (ex. "Château") sur un grand territoire reste aussi lent que d'extraire la couche entière sur ce territoire.
