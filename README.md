...
- **`territoire.py`** : résout région/département/commune via les couches Admin Express COG. Les couches admin sont téléchargées intégralement une fois par édition et mises en cache **sur disque** (`.cache/admin_*.gpkg`, ~480 Mo pour les communes) — mais **jamais chargées en entier en mémoire** : `search()` s'appuie sur un index léger (code INSEE + nom, sans géométrie) et `resolve()` ne lit que la ligne demandée depuis le cache disque (`pyogrio.read_dataframe(..., where=...)`). Avant ce découpage, tout le GeoDataFrame communes restait en mémoire pour la durée de vie du process, ce qui dépassait la limite RAM (~1 Go) de Streamlit Community Cloud.
...
- **Aperçu cartographique post-extraction** (`app.py::_build_preview` + `map_ui.py::render_extraction_preview`) : juste avant le téléchargement, un échantillon du résultat est affiché sur une carte (une couleur par couche, légende avec bascule d'affichage par couche). Volontairement **plafonné** (300 entités max par couche, 3000 au total, géométries simplifiées, toujours reprojetées en WGS84 pour l'affichage quel que soit le CRS de sortie choisi) et construit à partir d'un échantillon, pas du résultat complet — un gros territoire peut compter des centaines de milliers d'entités, bien trop pour un rendu navigateur raisonnable, et on veut éviter de reproduire en session_state le dépassement de mémoire déjà rencontré (cf. `territoire.py` ci-dessus). Le fichier téléchargé, lui, contient toujours l'intégralité des entités : seul l'aperçu est échantillonné.
